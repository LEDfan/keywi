...
LocalSecureStorage.prototype._setupNewPassword = function() {
  return new SetupNewPasswordDialog().then(userKey => Crypto.deriveKey(userKey, true)).then(encryptionKey => {
...
LocalSecureStorage.prototype._unlockStorage = function() {
  ...
  return new Promise(function(resolve, reject) {
    ...
    if (LocalSecureStorage.prototype._encryptionkey !== null) {
      ...
      self._hasEncryptionKey().then(function() {
 ...
 */
LocalSecureStorage.prototype.reencrypt = async function() {
  let data = await browser.storage.local.get();
  await this._unlockStorage();
  const dataToSave = {};
  ...
  for (const key of Object.keys(data)) {
    if (key.substr(0, prefixLength) === LocalSecureStorage.prototype._prefix) {
       ...
       * and do not re-encrypt the dummy value
       */
      const userKey = key.substr(prefixLength, key.length - prefixLength);
      if (userKey !== LocalSecureStorage.prototype._dummyValueKey) {
        dataToSave[userKey] = this._decrypt(data[key]);
      }
      this.delete(userKey); // remove from the Secure Storage
    }
  }
  ...
  await this._setupNewPassword();
  for (const ikey of Object.keys(dataToSave)) {
    const idata = dataToSave[ikey];
    this.set(ikey, idata); // will automatically use the new key
  }
  await Keywi._backend.reAssociate()
};
